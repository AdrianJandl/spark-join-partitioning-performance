...
object Main {
  ...
  def doDatasetJoin(sparkSession: SparkSession, repetitions: Int, bigDataPath: String, smallDataPath: String,
    ...
    val smallData: Dataset[Geoname] = sparkSession
      .read
      .option("header", "true")
      .schema(Encoders.product[Geoname].schema)
      .option("mode", "DROPMALFORMED")
      .csv(bigDataPath)
      .as[Geoname]
      .repartition(partitions, $"longitude")
    ...
    val partitionedBig = if (partitions > 0) bigData.repartition(partitions, $"latitude") else bigData
    metrics.+=("Partitions:>" + partitionedBig.rdd.getNumPartitions + "<")
    results.++=(metrics)

    partitionedBig.count()
    smallData.count()

    val start = System.nanoTime()
    val joined = partitionedBig.as("df1").join(smallData.as("df2"), $"df1.latitude" === $"df2.longitude")
    joined.count()
  ...
  def doRddJoin(sparkSession: SparkSession, repetitions: Int, bigDataPath: String, smallDataPath: String, outputPath: String, partitions: Int): mutable.MutableList[String] = {
    val results = mutable.MutableList[String]()
    val bigData: RDD[(Double, Iterable[String])] = sparkSession.sparkContext.textFile(bigDataPath, partitions).map(_
      .split(","))
      .filter(item => !item(0).startsWith("geo"))
      .map(item => (item(4).toDouble, item(1))).groupByKey()
    val smallData: RDD[(Double, Iterable[String])] = sparkSession.sparkContext.textFile(smallDataPath, partitions).map(_
      .split(","))
      .filter(item => !item(0).startsWith("geo"))
      .map(item => (item(5).toDouble, item(1))).groupByKey()
    ...
    bigData.count()
    smallData.count()
    //sparkSession.sparkContext.parallelize(metrics).coalesce(1).saveAsTextFile(outputPath + "metrics")

    val start = System.nanoTime()
    val joined: RDD[(Double, (Iterable[String], Iterable[String]))] = bigData.join(smallData)
    joined.count()
    val end = System.nanoTime()
    results.+=((end - start).toString)
    results
  ...
  def doDataFrameJoin(sparkSession: SparkSession, repetitions: Int, bigDataPath: String, smallDataPath: String, outputPath: String, partitions: Int, appendix: String): mutable.MutableList[String] = {
    ...
    metrics.+=("Number of workers: " + (sparkSession.sparkContext.getExecutorStorageStatus.length - 1))
    metrics.+=("Cores: " + Runtime.getRuntime.availableProcessors())
    val partitionedBigData: DataFrame = if (partitions > 0) bigData.repartition(partitions) else bigData
    ...
    val start = System.nanoTime()
    val joined = partitionedBigData.as("df1").join(smallData.as("df2"), $"df1.latitude" === $"df2.longitude")
    joined.count()
    val end = System.nanoTime()
    results.+=((end - start).toString)
    results
  ...
  def doManualBroadcastHashJoin(bigDataPath: String, smallDataPath: String, sparkSession: SparkSession,
    ...
    val results = mutable.MutableList[String]()
    val bigRDD: RDD[(Double, Iterable[String])] = sparkSession.sparkContext.textFile(bigDataPath, partitions).map(_.split
    (","))
      .filter(item => !item(0).startsWith("geo"))
      .map(item => (item(4).toDouble, item(1))).groupByKey()
    val smallRDD: RDD[(Double, Iterable[String])] = sparkSession.sparkContext.textFile(smallDataPath, partitions).map(_.split
    (","))
      .filter(item => !item(0).startsWith("geo"))
      .map(item => (item(5).toDouble, item(1))).groupByKey()
